...
function jadis_comment_form_submit_field($submit_field, $args){
  return $submit_field.((!is_user_logged_in() ? '</div>' : ''));
}
add_filter('comment_form_submit_field', 'jadis_comment_form_submit_field');
function jadis_new_excerpt_more( $more ) {
    return ' ...';
}
add_filter('excerpt_more', 'jadis_new_excerpt_more');
add_filter( 'post_thumbnail_html', 'jadis_remove_thumbnail_dimensions', 10, 3 );

function jadis_remove_thumbnail_dimensions( $html, $post_id, $post_image_id ) {
    $html = preg_replace( '/(width|height)=\"\d*\"\s/', "", $html );
    return $html;
}
/**
 * If the description is to long,the content is limited.
 */
